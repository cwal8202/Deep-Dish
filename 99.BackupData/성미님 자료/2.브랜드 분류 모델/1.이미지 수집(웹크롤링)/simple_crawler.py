...
search_list = {
    ...
    "롯데리아 햄버거 후기": "food/hamburger/lotteria",
    "롯데리아 햄버거 내돈내산": "food/hamburger/lotteria",
    
    "버거킹 햄버거 사진": "food/hamburger/burgerking",
    "버거킹 와퍼 실물": "food/hamburger/burgerking", 
    "버거킹 치킨버거": "food/hamburger/burgerking",
    "버거킹 햄버거 후기": "food/hamburger/burgerking",
    "버거킹 햄버거 내돈내산": "food/hamburger/burgerking",
    
    "쉑쉑버거 사진": "food/hamburger/shakeshack",
    "쉑쉑버거 실물": "food/hamburger/shakeshack",
    "쉑쉑버거 후기": "food/hamburger/shakeshack",
    
    "서브웨이 샌드위치 사진": "food/sandwich/subway",
    "서브웨이 실물": "food/sandwich/subway",
    "서브웨이 내돈내산": "food/sandwich/subway",
    "서브웨이 후기": "food/sandwich/subway",
    
    "에그드랍 사진": "food/sandwich/eggdrop",
    "에그드랍 실물": "food/sandwich/eggdrop",
    "에그드랍 내돈내산": "food/sandwich/eggdrop",
    
    "홍루이젠 샌드위치 사진": "food/sandwich/hongruijian", 
    "홍루이젠 후기": "food/sandwich/hongruijian",
    "홍루이젠 내돈내산": "food/sandwich/hongruijian",
    "대만 샌드위치 홍루 이젠": "food/sandwich/hongruijian",
    "홍루이젠": "food/sandwich/hongruijian",
    
    "노티드 도넛 사진": "food/dessert/knotted",
    "노티드 도넛 실물": "food/dessert/knotted",
    "노티드 도넛 선물": "food/dessert/knotted",
    "노티드 도넛 추천": "food/dessert/knotted",
    
    "몽슈슈 도지마롤": "food/dessert/monchouchou",
    "몽슈슈 케이크 실물": "food/dessert/monchouchou", 
    "몽슈슈 케이크 선물": "food/dessert/monchouchou", 
    
    "투썸플레이스 조각 케이크 사진": "food/dessert/twosome",
    "투썸플레이스 레드벨벳 케이크": "food/dessert/twosome",
    "투썸플레이스 시즌 조각 케이크 후기": "food/dessert/twosome",
    "투썸플레이스 아이스박스 조각 케이크": "food/dessert/twosome"
}
...
def url_encode(text):
    ...
    try:
        ...
        # fallback
        return text.replace(' ', '%20').replace('햄버거', '%ED%96%84%EB%B2%84%EA%B1%B0').replace('사진', '%EC%82%AC%EC%A7%84')
...
def download_images(image_urls, save_dir, keyword, max_images=200, start_index=0):
    """이미지 다운로드 - 시작 인덱스 추가"""
    ...
    print(f"💾 이미지 다운로드 시작 (목표: {max_images}장, 시작번호: {start_index})")
    ...
    for i, url in enumerate(image_urls):
        ...
        save_path = os.path.join(save_dir, f"{keyword_short}_{start_index + downloaded_count + 1:05d}.jpg")
...
def crawl_keyword(keyword, save_dir, max_images=200):
    ...
    # 여러 번 시도해서 최대한 많은 이미지 수집
    total_downloaded = 0
    attempt = 1
    max_attempts = 3
    
    while total_downloaded < max_images and attempt <= max_attempts:
        print(f"\n🔄 시도 {attempt}/{max_attempts} - 목표: {max_images - total_downloaded}장")
        
        driver = setup_driver()
        
        try:
            # 첫 시도가 아니면 다른 검색 방식 사용
            if attempt == 1:
                encoded_keyword = keyword.replace(' ', '+')
                search_url = f"https://www.google.com/search?q={encoded_keyword}&tbm=isch&hl=ko"
            elif attempt == 2:
                # 영어로 검색
                encoded_keyword = keyword.replace('햄버거', 'burger').replace('사진', 'photo').replace(' ', '+')
                search_url = f"https://www.google.com/search?q={encoded_keyword}&tbm=isch&hl=en"
            else:
                # 다른 파라미터로 검색
                encoded_keyword = keyword.replace(' ', '+')
                search_url = f"https://www.google.com/search?q={encoded_keyword}&tbm=isch&hl=ko&safe=off"
            
            print(f"🌐 검색: {keyword} (시도 {attempt})")
            ...
            print(f"⏳ 자연스러운 대기: {wait_time:.1f}초")
            ...
            image_urls = get_image_urls(driver, max_images=remaining*3)
            
            if not image_urls:
                print(f"❌ 시도 {attempt}: 수집된 URL이 없습니다.")
                attempt += 1
                continue
            ...
            downloaded = download_images(
                image_urls, save_dir, f"{keyword}_attempt{attempt}", 
                max_images=remaining, start_index=existing_count
            )
            
            total_downloaded += downloaded
            print(f"✅ 시도 {attempt} 완료: {downloaded}장 다운로드 (총: {total_downloaded}장)")
            
            if downloaded < 10:  # 너무 적게 다운로드되면 다음 시도
                print("⚠️ 다운로드가 적어서 다른 방법으로 재시도합니다.")
                attempt += 1
            else:
                break
            ...
            if attempt <= max_attempts:
                ...
                print(f"⏳ 다음 시도까지 대기: {wait_time:.1f}초")
                time.sleep(wait_time)
    
    print(f"🎯 최종 결과: {total_downloaded}장 수집 완료")
...
def main():
    ...
    # 목표 설정 (현실적으로)
    brand_targets = {
        "food/hamburger/lotteria": 10000,
        "food/hamburger/burgerking": 10000,
        "food/hamburger/shakeshack": 10000,
        
        "food/sandwich/subway": 10000,
        "food/sandwich/eggdrop": 10000,
        "food/sandwich/hongruijian": 10000,
        
        "food/dessert/knotted": 10000,
        "food/dessert/monchouchou": 10000,
        "food/dessert/twosome": 10000
    }
    ...
    # 전체 키워드 리스트 출력
    print(f"\n📋 총 {len(search_list)}개 키워드:")
    for i, (keyword, folder) in enumerate(search_list.items(), 1):
        brand = folder.split('/')[-1]
        print(f"  {i:2d}. {keyword} → {brand}")
    ...
    for keyword, folder_path in search_list.items():
        ...
        if current_counts[folder_path] >= brand_targets[folder_path]:
            print(f"\n⏩ ({completed_keywords}/{len(search_list)}) {folder_path} 브랜드 목표 달성 (건너뛰기)")
            continue
        
        remaining = brand_targets[folder_path] - current_counts[folder_path]
        download_count = min(remaining, 4000)  # 키워드당 최대 150장
        
        brand_name = folder_path.split('/')[-1]
        category = folder_path.split('/')[1]
        
        print(f"\n" + "="*60)
        print(f"📍 진행상황: {completed_keywords}/{len(search_list)} 키워드")
        print(f"📂 {category} > {brand_name}")
        print(f"🔍 키워드: '{keyword}'")
        print(f"📊 현재: {current_counts[folder_path]}/{brand_targets[folder_path]}장 (목표: +{download_count}장)")
        
        # 사용자 확인
        user_input = input(f"🤔 '{keyword}' 크롤링을 시작할까요? (y/n/skip): ").lower().strip()
        if user_input == 'n':
            print("❌ 사용자가 중단을 선택했습니다.")
            break
        elif user_input == 'skip':
            print("⏭️ 이 키워드를 건너뜁니다.")
            continue
        ...
        progress = (completed_keywords / len(search_list)) * 100
        print(f"📈 전체 진행률: {progress:.1f}% ({completed_keywords}/{len(search_list)} 키워드 완료)")
        print(f"📊 총 수집: {total_downloaded}장")
        
        # 다음 키워드로 넘어가기 전 확인
        if completed_keywords < len(search_list):
            next_keyword = list(search_list.keys())[completed_keywords]
            print(f"⏭️  다음 키워드: '{next_keyword}'")
            
            continue_input = input("계속하시겠습니까? (y/n): ").lower().strip()
            if continue_input == 'n':
                print("❌ 사용자가 중단을 선택했습니다.")
                break
    ...
    print(f"📋 처리된 키워드: {completed_keywords}/{len(search_list)}개")
