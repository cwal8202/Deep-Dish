...
def load_models(model_dir: str, timestamp: str):
    ...
    eff_model = build_transfer_model_with_gap_output(EfficientNetB0, num_classes)
    eff_model.load_weights(os.path.join(model_dir, f"effnet_model_best_{timestamp}.h5"))

    res_model = build_transfer_model_with_gap_output(ResNet50V2, num_classes)
...
print("✅ Ensemble 모델 (EffNet, ResNet, XGBoost) 로딩 완료.")
# ----------------------------------------------------
...
def predict_image(path: str):
    ...
    # 미리 로드된 전역 모델들을 사용
    feat_eff, p_eff = EFF_MODEL(inp_eff, training=False)
    feat_res, p_res = RES_MODEL(inp_res, training=False)

    p_cnn = (p_eff + p_res) / 2.0
    feat = np.hstack([feat_eff.numpy(), feat_res.numpy()])
    ...
    probs = {IDX2LABEL[i]: float(ensemble[i]) for i in range(len(ensemble))}
