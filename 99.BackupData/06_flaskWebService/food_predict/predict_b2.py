...
# 이미지 예측 (최적화 적용)
def predict_image(path: str, eff_model, index_to_label: dict):
    ...
    probs = {index_to_label[i]: float(prob[i]) for i in range(len(prob))}
...
def run_prediction(saved_info_list: List[tuple[int, str]]) -> List[Dict[str, Any]]:
    ...
    for image_id, img_path in saved_info_list:
        ...
        label, conf, probs = predict_image(full_path, eff_model, idx2label)
